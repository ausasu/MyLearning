Shader "ToonShader/ToonShaderPlus01" {
    Properties {
        [Header(Texture)]
    ...
    SubShader {
        ...
        Pass {
            ...
            v2f vert(a2v v) {
                ...
                float3 normal = mul((float3x3)UNITY_MATRIX_IT_MV, v.tangent.xyz); // 转换到视角方向
                float2 OffNormal = TransformViewToProjection(normal.xy);
                if(_ToggleOutline){
                    pos.xy += 0.001 *  _Outline * OffNormal.xy * v.vertColor.a;//顶点色a通道控制粗细
                    o.pos = pos;
            ...
            fixed4 frag(v2f i) : SV_TARGET {
                float3 color;
                if(_ToggleOutline){
                    color = _OutlineColor.rgb * i.vertColor;
                }else{
