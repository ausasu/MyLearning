Shader "ToonShader/ToonShaderV02" {
    Properties {
        [Header(Texture)]
        ...
        _SpecularThreshold          ("高光大小", Range(0.001, 1)) = 0.1
    ...
    SubShader {
        ...
        Pass {
            ...
            v2f vert(a2v v) {
                ...
                float3 normal = mul((float3x3)UNITY_MATRIX_IT_MV, v.vertColor.rgb * 2.0 -1.0); // 转换到视角方向
                float2 OffNormal = TransformViewToProjection(normal.xy);
                if(_ToggleOutline){
                    pos.xy += 0.001 *  _Outline * OffNormal.xy;
                    o.pos = pos;
                    //o.vertColor = v.vertColor.rgb;
                }else{
                    o.pos = UnityObjectToClipPos(float4(v.vertex.xyz + v.normal * _Outline * 0.0001, 1));
                }
